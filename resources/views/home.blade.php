        ...
        <div class="div-block-367">
            ...
            @if(isset($rookie)) 
                <div class="recondation-video-card">
                    <a href="{{route('play', $rookie->id)}}" class="video-vignette-recom">
                        @if($rookie->thumbnail)
                            <img src="{{asset('vids/thumbnails/') }}/{{$rookie->thumbnail}}" class="vignette-video-sugg">
                        @else
                            <img src="{{asset('img/grille.png')}}" class="vignette-video-sugg">
                        @endif
                    </a>
                    @php 
                        $date = explode(":",$rookie->duration);
                        
                        if($date && count($date) > 0)
                            if($date[0] = "00")
                                $rookie->duration = $date[1] . ":" . $date[2];
                    @endphp
                    <p class="durationVideo2">{{$rookie->duration}}</p>
                    <div class="vignette-vid-info-recom">
                        <div class="div-block-295">
                        <a href="{{route('play',$rookie->id)}}" class="titre-v-c-recom" style="text-decoration:none">{{$rookie->main_title}}</a>
                            ...
                            <div class="bull-name-stat">
                                ...
                                <div class="div-block-292">
                                    ...
                                    <div>
                                        ...
                                        @if(intval(abs(strtotime("now") - strtotime($rookie->created_at))/ 86400) == 0)
                                        @if(intval(abs(strtotime("now") - strtotime($rookie->created_at))/ 3600) > 0)
                                        <p class="publication-info-recom">{{intval(abs(strtotime("now") - strtotime($rookie->created_at))/3600)}} hours ago </p>
                                        @else(intval(abs(strtotime("now") - strtotime($rookie->created_at))/ 3600) == 0)
                                        <p class="publication-info-recom">{{intval(abs(strtotime("now") - strtotime($rookie->created_at))/60)}} minutes ago </p>
                                        @endif
                                        @elseif(intval(abs(strtotime("now") - strtotime($rookie->created_at))/ 86400) == 1)
                                        <p class="publication-info-recom">Yesterday at {{strftime("%H:%M", strtotime($rookie->created_at))}}</p>
                                        @elseif(intval(abs(strtotime("now") - strtotime($rookie->created_at))/ 86400) >= 2 && intval(abs(strtotime("now") - strtotime($rookie->created_at))/ 86400) <= 27)
                                        <p class="publication-info-recom"> {{intval(abs(strtotime("now") - strtotime($rookie->created_at))/ 86400)}} days ago </p>
                                        @elseif(intval(abs(strtotime("now") - strtotime($rookie->created_at))/ 86400) > 30)
                                        <p class="publication-info-recom">{{intval(abs(strtotime("now") - strtotime($rookie->created_at))/ 2592000) }} months ago</p>
                                        @else(intval(abs(strtotime("now") - strtotime($rookie->created_at))/ 2592000) > 12)
                                        <p class="publication-info-recom">{{intval(abs(strtotime("now") - strtotime($rookie->created_at))/ 2592000 * 12) }} years ago</p>
                                        @endif
                                    </div>
                    ...
                    <div class="d-flex justify-content-between blockPlayist" >
                        ...
                        <a  href="{{route('playlist.remove', $rookie->id)}}" data-toggle="tooltip" data-placement="top" title="Remove to my playlist"><img src="{{ asset('img/report-Orange.svg') }}"  alt="Remove to my playlist"></a>
                    </div>
                    @else
                    <div class="d-flex justify-content-between blockPlayist" >
                        <p class="numberviewsSuggestion"> </p>
                        <a href="{{route('playlist.add', $rookie->id)}}" data-toggle="tooltip" data-placement="top" title="Add to my playlist"><img src="{{ asset('img/report-gris.svg') }}"  alt="Add to my playlist"></a>
                    </div>
                    @endif
                </div>
            @endif
                ...
                @foreach($playlists as $playlist)
                    <div class="swiper-slide suggestionSwipe">
                        <div class="recondation-video-card">
                            ...
                            @endphp
                            <p class="durationVideo2">{{$playlist->duration}}</p>
                            <div class="vignette-vid-info-recom">
                                <div class="div-block-295">
                                    ...
                                    <div class="bull-name-stat">
                                        ...
                                        <div class="div-block-292">
                                            ...
                                            <div>
                                                ...
                                                <p class="publication-info-recom">{{intval(abs(strtotime("now") - strtotime($playlist->created_at))/3600)}} hours ago </p>
                                                @else(intval(abs(strtotime("now") - strtotime($playlist->created_at))/ 3600) == 0)
                                                <p class="publication-info-recom">{{intval(abs(strtotime("now") - strtotime($playlist->created_at))/60)}} minutes ago </p>
                                                @endif
                                                @elseif(intval(abs(strtotime("now") - strtotime($playlist->created_at))/ 86400) == 1)
                                                <p class="publication-info-recom">Yesterday at {{strftime("%H:%M", strtotime($playlist->created_at))}}</p>
                                                ...
                                                <p class="publication-info-recom"> {{intval(abs(strtotime("now") - strtotime($playlist->created_at))/ 86400)}} days ago </p>
                                                @elseif(intval(abs(strtotime("now") - strtotime($playlist->created_at))/ 86400) > 30)
                                                <p class="publication-info-recom">{{intval(abs(strtotime("now") - strtotime($playlist->created_at))/ 2592000) }} months ago</p>
                                                @else(intval(abs(strtotime("now") - strtotime($playlist->created_at))/ 2592000) > 12)
                                                <p class="publication-info-recom">{{intval(abs(strtotime("now") - strtotime($playlist->created_at))/ 2592000 * 12) }} years ago</p>
                                                @endif
                                            </div>
                                            <div class="div-block-294">
                                                <p class="text-block-301">100k</p>
